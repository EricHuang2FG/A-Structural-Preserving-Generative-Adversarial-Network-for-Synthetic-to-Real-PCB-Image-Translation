RAW_DATA_DIR: str = "data/open-schematics"

BOARD_RENDER_HEIGHT: int = 1020
BOARD_RENDER_WIDTH: int = 1020
TARGET_IMAGE_SIZE: int = 256  # square
RENDER_QUALITY: str = "high"
DIFFERENTIAL_RENDERING_QUALITY: str = "basic"
DIFFERENTIAL_THRESHOLD: int = 18
DIFFERENTIAL_RENDERING_COLOUR_GROUP_MARGIN_PX: int = 12
MIN_BLOB_AREA: int = 20

BOARD_SIDE_TOP: str = "top"
BOARD_SIDE_BOTTOM: str = "bottom"

REFERENCE_TO_CLASS_MAPPING: dict[str, str] = {
    "R": "resistor",  # R1, R2, ...
    "C": "capacitor",
    "L": "inductor",
    "U": "ic",
    "Q": "transistor",
    "D": "diode",
    "J": "connector",
    "P": "connector",
    "SW": "switch",
    "Y": "crystal",
    "F": "fuse",
    "K": "relay",
}
DEFAULT_CLASS: str = "other"

CLASS_TO_SEMANTIC_INDEX_MAPPING: dict[str, int] = {
    "background": 0,
    "resistor": 1,
    "capacitor": 2,
    "inductor": 3,
    "ic": 4,
    "transistor": 5,
    "diode": 6,
    "connector": 7,
    "switch": 8,
    "crystal": 9,
    "fuse": 10,
    "relay": 11,
    "other": 12,
}

SEED: int = 42
EARLY_STOPPING_PATIENCE: int = 6

TITLE_FONTSIZE: int = 14
TITLE_FONTWEIGHT: str = "bold"
AXIS_LABEL_FONTSIZE: int = 13
LEGEND_FONTSIZE: int = 12

MODEL_NAME_TEMPLATE: str = (
    "{{ model_name }}_bs{{ batch_size }}_lr{{ learning_rate }}_epoch{{ epoch }}.model"
)
TRAINING_CURVE_FILE_NAME_TEMPLATE: str = (
    "{{ model_name }}_bs{{ batch_size }}_lr{{ learning_rate }}_{{ type }}.pdf"
)
SEGMENTOR_MODEL_CHECKPOINTS_DIRECTORY: str = (
    "/scratch/huan4397/out/segmentor/checkpoints"
)
SEGMENTOR_MODEL_BEST_MODEL_DIRECTORY: str = "/scratch/huan4397/out/segmentor/best"
SEGMENTOR_MODEL_TRAINING_CURVE_DIRECTORY: str = (
    "/scratch/huan4397/out/segmentor/training_curves"
)
SEGMENTOR_MODEL_SWEEP_CURVE_DIRECTORY: str = (
    "/scratch/huan4397/out/segmentor/sweep_curves"
)

SPRESGAN_MODEL_CHECKPOINTS_DIRECTORY: str = (
    "/scratch/huan4397/out/spresgan_v6_final/checkpoints"
)
SPRESGAN_MODEL_BEST_MODEL_DIRECTORY: str = (
    "/scratch/huan4397/out/spresgan_v6_final/best"
)
SPRESGAN_MODEL_TRAINING_CURVE_DIRECTORY: str = (
    "/scratch/huan4397/out/spresgan_v6_final/training_curves"
)
